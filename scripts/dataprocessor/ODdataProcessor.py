...
class DatasetProcessor:
    ...
    def adjust_annotation(anno_lines, roi, img_width, img_height, crop_width, crop_height):
        ...
        for line in anno_lines:
            ...
            if len(parts) < 5:
                continue
            obj_class, x_center, y_center, width, height = map(float, parts[:5])
            ...
            if x1 <= abs_x_center <= x2 and y1 <= abs_y_center <= y2:
                new_x_center = (abs_x_center - x1) / crop_width
                new_y_center = (abs_y_center - y1) / crop_height
                new_width = abs_width / crop_width
                new_height = abs_height / crop_height

                new_annotations.append(f"{int(obj_class)} {new_x_center} {new_y_center} {new_width} {new_height}\n")
